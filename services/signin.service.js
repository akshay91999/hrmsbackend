    ...
    bcrypt.compare(password, base.password)
        .then(async doMatch => {
            if (doMatch) {
                ...
                const { p_change } = Log
                const { dp_id } = Dep
                const { ds_id, designation } = Des

                return res.cookie('access_token', token, { httpOnly: true }).json({ token, user: { id, p_change, user_type, name, email, dp_id, ds_id, designation } })
            }
            else {
